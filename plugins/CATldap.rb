...
class CATldap
  ...
  def initialize(*args)
    super
    @cat_ldap = LdapHelper.load_from_yaml_file(
      LdapHelper::DEFAULT_CONFIG_FILE, 'cat'
    )
    @oit_ldap = LdapHelper.load_from_yaml_file(
  ...
  def execute(m, query)
    ...
    cat_result = @cat_ldap.search(attribute, query)
    ...
    cat_result.each do |cat_entry|
      ...
      if cat_entry[:uniqueidentifier].empty?
        ...
        uniqueid = 'P' + uniqueid if uniqueid !~ /^P/i
        oit_result = @oit_ldap.search('uniqueidentifier', uniqueid)
        if !oit_result
